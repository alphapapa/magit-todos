...
+  Magit only works inside a git repo, but to view to-dos outside of one, we can make Magit think it's in a repo by using [[https://github.com/nicferrier/emacs-noflet][noflet]], like this:

#+BEGIN_SRC elisp
  (defun magit-todos-list-anywhere ()
    "Open to-dos list outside of git repo with `magit-todos-list'."
    (interactive)
    (noflet ((magit-toplevel (&rest _) default-directory))
      (magit-todos-list)))
#+END_SRC
