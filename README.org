...
Helm and Ivy are also supported.  Note that the =helm= and =ivy= packages are not required, nor does this package depend on them; they are only used if present.

+  =helm-magit-todos= :: Display the to-do list with Helm.
+  =ivy-magit-todos= :: Display the to-do list with Ivy.
