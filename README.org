...
** Warning
This package is not supposed to be tightly integrated with ~hl-todo-mode~!
It can use `hl-todo` keywords for convinience, but that's it.

For example, note that for ~magit-todos~ to work, *by default* you need to put a colon at the end of the keyword:
- ~; HACK: I should get rid of this someday~ - will get marked
- ~; HACK THESE LOCAL VARIABLES!~ - will not
